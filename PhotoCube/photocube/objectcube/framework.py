...
class ObjectCube:
    ...
    def __init__(self, dataAccessType, dbName = None, language=None):
        ...
        print('dataAccessType=' + str(browser_config.getValue('dataAccessType')))
        ...
        #Parameters.getParameters().add( "PostgreSQL_database", "ObjectCube" )
    ...
    def search_words(self, values):
        print("search_words")
        result = self.hub.searchText(values)
        print 'TagSetId: ' + str(result[0])
        print 'DimensionId: ' + str(result[1])
        self.hub.getHub()
        return result

    def search_rgb(self, name):
        print 'search_rgb'
        result = self.hub.searchRGB(name)
        print 'search_rgb: TagSetId: ' + str(result[0])
        print 'search_rgb: DimensionId: ' + str(result[1])
        return result        
    
    def search_image(self, url, limit, tagsetId, dimension):
        print 'search_image'
        red = 0
        green = 0
        blue = 0
        count = 0
        with WandImage(filename=url) as image:
            for row in image:
                for col in row:
        ...
        print 'RGB: ' + RGB
        name = RGB + ";" + limit + ";" + str(tagsetId) + ";" + str(dimension)
        result = self.hub.searchRGB(name)
        print 'search_rgb: TagSetId: ' + str(result[0])
        print 'search_rgb: DimensionId: ' + str(result[1])
        return result 
...
objectCubeService = ObjectCube(POSTGRESQL)
