...
from photocube.objectcube import TagSet
from photocube.devices import mouse_handler
from photocube.common import Hoverable, font, tagTexture
...
def convert_cluster_pos_to_screen_pos(pos):
    ...
    if pos[0] is not None:
        x = (pos[0] * (photocube.cube.cubeService.get_max_image_space() + photocube.cube.cubeService.get_max_image_size()))
    
    if pos[1] is not None:
        y = (photocube.cube.cubeService.get_max_image_size()/2) + pos[1] * (photocube.cube.cubeService.get_max_image_size() + photocube.cube.cubeService.get_max_image_space() )

    if pos[2] is not None:
        z = ( pos[2] * (photocube.cube.cubeService.get_max_image_space() + photocube.cube.cubeService.get_max_image_size()))
...
class Image( ImageNodePath, Hoverable ):
    ...
    def default_move_image_function(self, np, location, old_pos, new_pos, cluster_index):
        ...
        # add the stackspace. 
        new_pos = ( new_pos[0], new_pos[1] + cluster_index * photocube.cube.cubeService.get_cluster_stack_space(), new_pos[2] )
    ...
    def test_add_image_function(self,np, pos):
        ...
        # add the stack space.
        new_pos = ( new_pos[0], new_pos[1] + self.cluster_index * photocube.cube.cubeService.get_cluster_stack_space(), new_pos[2] )
    ...
    def load_data(self):
        ...
        for tag in obj.getTags():
            ...
            if tagset.accessId == TagSet.ACCESS_TYPE.USER: 
                if tagset.name != 'Slide text': #get rid of long tags in the master defence presentation TAKE OUT AND FIND A WAY TO SHOW NOT LONG TAGS
    ...
    def position(self, animate=False):
        ...
        if animate:
            ...
            # add the stack space
            self.np.setY( self.np.getY() + (self.cluster_index * photocube.cube.cubeService.get_cluster_stack_space()) )
    ...
    def scale_image(self, scale_factor=None, scale_tags=False):
        ...
        if scale_factor is None:
            max_image_size = photocube.cube.cubeService.get_max_image_size()
        
        else:
            max_image_size = scale_factor
        
        max_image_space = photocube.cube.cubeService.get_max_image_space()
