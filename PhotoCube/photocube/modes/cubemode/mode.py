...
import photocube
from photocube.cube.labels import DrillUpLabel
from photocube.dialogs import CellInfoDialog
from photocube.common import X_AXIS, Y_AXIS, Z_AXIS
from photocube.configuration import browser_config
from photocube.actions import actionManager, ActionAxis, ACTION_DRILL_DOWN, ACTION_DRILL_UP
...
from photocube.ui.coremenu.context import make_context_item, FRAME_COLOR
from photocube.ui.coremenu import context_menu_handler
...
class CubeMode( AbstractMode ):
    ...
    def __init__(self, name):
        ...
        # This task is used for dragging the mouse to relocate the camera.
        #self.mouseDraggingTask = None
        #self.mouseDraggingTask = taskMgr.add(self.mouseDraggingTask, 'mouseDraggingTask')
        #self.is_dragging = False
        #self.mouse_click_location = None
        base.accept('t', self.printSelectedCells)




    def mouseDraggingTask(self, task):
        if self.is_dragging and self.mouse_click_location:
            current_mouse_location = (base.mouseWatcherNode.getMouseX(),base.mouseWatcherNode.getMouseY())
            delta_x = current_mouse_location[0] - self.mouse_click_location[0]
            delta_y = current_mouse_location[1] - self.mouse_click_location[1]
            v = LVecBase3f(-delta_x, 0, -delta_y)
            base.camera.setPos(base.camera.getPos() + (v*(self.drag_scale)))
            self.mouse_click_location = (base.mouseWatcherNode.getMouseX(),base.mouseWatcherNode.getMouseY())
        return task.cont


    
    def mouseOneEmptyClickDown(self, p):
        from photocube.ui.coremenu import context_menu_handler
        context_menu_handler.close_open()
        self.is_dragging = True
        self.mouse_click_location = (base.mouseWatcherNode.getMouseX(),base.mouseWatcherNode.getMouseY())
        #print self.mouse_click_location
        #taskMgr.add(self., 'MyTaskName')


    def mouseOneEmptyClickUp(self, p):
        self.is_dragging = False
        self.mouse_click_location = None
        #taskMgr.remove('mouseDraggingTask')
        #self.mouseDraggingTask = None
    ...
    def disable(self):
        ...
        self.np.hide()
        self.np.setX(-200000)
        photocube.dialogs.dialogService.closeInfoDialogs()
        ...
        taskMgr.remove(self.mouseDraggingTask)
    ...
    def initalize(self):
        ...
        # Mouse down none and mouse down up is used for scroll viewing in the cube.
        base.accept('CLICK_MOUSE1_None', self.mouseOneEmptyClickDown)
        base.accept('CLICK_MOUSE1_None-up', self.mouseOneEmptyClickUp)
        ...
        #photocube.menu.pluginButtonContainer.createButton("Random", self.onRandomClick)
        #photocube.pluginButtonContainer.createButton("Freefloat", self.onFreefloatClick)
        #photocube.menu.pluginButtonContainer.createButton("Reset view", self.onCameraReset)
        photocube.ui.coremenu.pluginButtonContainer.createButton("Clear", self.clearCube)
        #self.toggleMenuButton = photocube.ui.coremenu.pluginButtonContainer.createButton("Hide menu", self.toggelRightMenu)
        

        # TODO: This is for the left menu.. the code for that menu should be moved here!!!!1
    ...
    def load(self, cube):        
        ...
        # Draw x_axis
        if photocube.cube.cubeService.get_x() is not None:
        ...
        if photocube.cube.cubeService.get_y() is not None:
            if browser_config['show_cube_lines'] is not None and browser_config['show_cube_lines'].lower() == 'true':
                self.__drawYAxis()
            self.__drawYLabels()
        else:
        ...
        if photocube.cube.cubeService.get_z() is not None:
            if browser_config['show_cube_lines'] is not None and browser_config['show_cube_lines'].lower() == 'true':
                self.__drawZAxis()
            self.__drawZLabels()
        else:
    ...
    def foo(self):
        ...
        """
        photocube.cube.cubeService.get_x().setLevel( photocube.cube.cubeService.get_x().getLevel()+1 )
        photocube.cube.cubeService.draw()
        self.__resetNormalVectors()
        self.__positionCamera()
    ...
    def scale(self, arg):
        if arg == 'up':
            photocube.cube.cubeService.set_max_image_size( photocube.cube.cubeService.get_max_image_size() + 0.1 )
            photocube.cube.cubeService.set_max_image_space( photocube.cube.cubeService.get_max_image_space() + 0.05 )
        else:
            if photocube.cube.cubeService.get_max_image_size() >=0:
                photocube.cube.cubeService.set_max_image_size( photocube.cube.cubeService.get_max_image_size() - 0.1 )
                photocube.cube.cubeService.set_max_image_space( photocube.cube.cubeService.get_max_image_space() - 0.05 )
    ...
    def clearCube(self):
        ...
        photocube.cube.cubeService.view_on_x( None )
        photocube.cube.cubeService.view_on_y( None )
        photocube.cube.cubeService.view_on_z( None )
        photocube.cube.cubeService.draw()
    ...
    def onRandomClick(self):
        ...
        tagsets = photocube.objectcube.objectCubeService.getAllTagsets(True)
        
        for n in range(3):
            if photocube.cube.cubeService.get_x() is None:
                ...
                if len(dims) > 0:
                    ...
                    photocube.cube.cubeService.view_on_x(dims[0])
                else:
                    #print 'viewing tagset', randomTagset.name
                    photocube.cube.cubeService.view_on_x(randomTagset)
            

            if photocube.cube.cubeService.get_z() is None:
                r = random.randint(1, len(tagsets)-1)
                randomTagset = tagsets[r]
                dims = randomTagset.getPersistentDimensions()
                ...
                if len(dims) > 0:
                    print 'Viewing dim', dims[0]
                    photocube.cube.cubeService.view_on_z(dims[0])
                else:
                    print 'viewing tagset', randomTagset.name
                    photocube.cube.cubeService.view_on_z(randomTagset)


            if photocube.cube.cubeService.get_y() is None:
                r = random.randint(1, len(tagsets)-1)
                randomTagset = tagsets[r]
                dims = randomTagset.getPersistentDimensions()
                ...
                if len(dims) > 0:
                    print 'Viewing dim', dims[0]
                    photocube.cube.cubeService.view_on_y(dims[0])
                else:
                    print 'viewing tagset', randomTagset.name
                    photocube.cube.cubeService.view_on_y(randomTagset)
        ...
        if axisFrom == Z_AXIS:
            ...
            if AxisTo == X_AXIS:
                ...
                if z.is_hierarchy():
                    ...
                    photocube.cube.cubeService.view_on_x( z.tagset )
                    if not curr_node is None:
                        photocube.cube.cubeService.coordinate.x.drill_down( curr_node )
                        parentNode = photocube.cube.cubeService.coordinate.z.get_parent_node_for_current()
            ...
            if AxisTo == Y_AXIS:
                ...
                if z.is_hierarchy():
                    ...
                    photocube.cube.cubeService.view_on_y( z.tagset )
                    if not curr_node is None:
                        photocube.cube.cubeService.coordinate.y.drill_down( curr_node )
                        parentNode = photocube.cube.cubeService.coordinate.z.get_parent_node_for_current()
        ...
        if axisFrom == Y_AXIS:
            ...
            if AxisTo == X_AXIS:
                ...
                if y.is_hierarchy():
                    ...
                    photocube.cube.cubeService.view_on_x( y.tagset )
                    if not curr_node is None:
                        photocube.cube.cubeService.coordinate.x.drill_down( curr_node )
                        parentNode = photocube.cube.cubeService.coordinate.y.get_parent_node_for_current()
            ...
            if AxisTo == Z_AXIS:
                ...
                if y.is_hierarchy():
                    ...
                    photocube.cube.cubeService.view_on_z( y.tagset )
                    if not curr_node is None:
                        photocube.cube.cubeService.coordinate.z.drill_down( curr_node )
                        parentNode = photocube.cube.cubeService.coordinate.y.get_parent_node_for_current()
        ...
        if axisFrom == X_AXIS:
            ...
            if AxisTo == Z_AXIS:
                ...
                if x.is_hierarchy():
                    ...
                    photocube.cube.cubeService.view_on_z( x.tagset )
                    if not curr_node is None:
                        photocube.cube.cubeService.coordinate.z.drill_down( curr_node )
                        parentNode = photocube.cube.cubeService.coordinate.x.get_parent_node_for_current()
            ...
            if AxisTo == Y_AXIS:
                ...
                if x.is_hierarchy():
                    ...
                    photocube.cube.cubeService.view_on_y( x.tagset )
                    if not curr_node is None:
                        photocube.cube.cubeService.coordinate.y.drill_down( curr_node )
                        parentNode = photocube.cube.cubeService.coordinate.x.get_parent_node_for_current()
    ...
    def OnViewTagset(self, axis, tagset):
        ...
        if axis == X_AXIS:
            photocube.cube.cubeService.view_on_x(tagset)
            self.rightMenu.setXViewText(tagset.name + " tagset")

        elif axis == Y_AXIS:
            photocube.cube.cubeService.view_on_y(tagset)
            self.rightMenu.setYViewText(tagset.name + " tagset")

        elif axis == Z_AXIS:
            photocube.cube.cubeService.view_on_z(tagset)
    ...
    def __drawXAxis(self):
        ...
        if x_len > 0:
            ...
            ls_x.setThickness( 1 )
            n_points = x_len
            n_lineLength = (n_points * photocube.cube.cubeService.get_max_image_size()) + ((n_points-1) * photocube.cube.cubeService.get_max_image_space())
    ...
    def __drawXLabels( self ):
        ...
        for label in x_labels:
            ...
            if label.get_hierarchy():
                node = photocube.cube.cubeService.get_x().get_node_by_label( label.__str__() )
                num_branches = len(node.getBranches())
                aggr_image_count = len( photocube.cube.cubeService.get_x().get_node_by_label( label.__str__() ).getObjectIds())
    ...
    def __drawYLabels( self ):
        ...
        y = photocube.cube.cubeService.get_y()
    ...
    def __drawZLabels( self ):
        ...
        z = photocube.cube.cubeService.get_z()
    ...
    def __positionCamera(self):
        ...
        if photocube.cube.cubeService.get_x() is not None:
            num_x_nodes =  photocube.cube.cubeService.get_x().get_node_count()
            # Calculating the center of the line.
            x_pos = (  photocube.cube.cubeService.get_max_image_size() * num_x_nodes )
            x_pos += photocube.cube.cubeService.get_max_image_space() * (num_x_nodes-1)
            x_pos = (x_pos / 2.0)
            base.camera.setX( x_pos )

        if photocube.cube.cubeService.get_y() is not None:
            num_y_nodes =  photocube.cube.cubeService.get_y().get_node_count()
    
        if photocube.cube.cubeService.get_z() is not None:
            num_z_nodes =  photocube.cube.cubeService.get_z().get_node_count()
            # Calculating the center of the line.
            z_pos = (  photocube.cube.cubeService.get_max_image_size() * num_z_nodes )
            z_pos += photocube.cube.cubeService.get_max_image_space() * (num_z_nodes - 1)
            z_pos = (z_pos / 2.0)
        ...
        if photocube.cube.cubeService.get_x() is not None:
            x_pos = ( photocube.cube.cubeService.get_max_image_size() * num_x_nodes ) # what happens when we rotade?
            x_pos += photocube.cube.cubeService.get_max_image_space() * (num_x_nodes - 1 )
            x_pos = (x_pos / 2.0)
            self.np.setX(-x_pos)
        ...
        if photocube.cube.cubeService.get_y() is not None:
            y_pos = ( photocube.cube.cubeService.get_max_image_size() * num_y_nodes )
            y_pos += photocube.cube.cubeService.get_max_image_space() * (num_y_nodes - 1)
            y_pos = (y_pos / 2.0)
            self.np.setY(-y_pos)
    ...
    def __showImages(self):
        ...
        photocube.image.image_event_service.clear_messages()
        ...
        for cell in cells.values():
            ...
            while num_images > 0 and len(visible_cell_images) > 0:
                ...
                photocube.image.image_event_service.add_message( image.getClickMessageLeft(), self.on_image_click_left )
                photocube.image.image_event_service.add_message( image.getClickMessageRight(), self.on_image_click_right )
           
                # Get the mouse over messages from the image.
                photocube.image.image_event_service.add_message( image.get_mouseover_message(), self.on_image_mouse_over )
                photocube.image.image_event_service.add_message( image.get_mouseleave_message(), self.on_image_mouse_leave )
                num_images -= 1
                cluster_index += 1
        ...
        if not self.cubeMouseRotater.isRotating() and not self.floatingCameraHandler.isEnabled():
            ...
            for image in self.images[ location ]:
                if image.get_id() == id:
                    ...
                    photocube.dialogs.dialogService.openCellInfoDialog(image.getCellKey(), len(cell.getImages()), clusterTags)
                    break
    ...
    def on_image_click_right(self, np):
        print photocube.ui.coremenu.context_menu_handler.open_image_context()
    ...
    def event_on_click_label_x(self, np, mouse = None):
        ...
        clicked_node = photocube.cube.cubeService.get_x().get_node_by_label( clicked_label.title )
        ...
        photocube.cube.cubeService.get_x().drill_down( clicked_node )
        photocube.cube.cubeService.coordinate.constructCube()
        self.load( photocube.cube.cubeService.coordinate.cube )
        ...
        # Get the parent of the node that was cliecked.
        parent_node = photocube.cube.cubeService.get_x().get_parent_for_node( clicked_node )
    ...
    def on_label_parent_x_click(self, e, mouse_button=None):
        print "---on_label_parent_x_click---"
        x = photocube.cube.cubeService.get_x()
    ...
    def event_on_click_label_z(self, np, mouse_button=None):
        ...
        clicked_node = photocube.cube.cubeService.get_z().get_node_by_label( clicked_label.title )
        ...
        photocube.cube.cubeService.get_z().drill_down( clicked_node )
        #self.construct_cube()
        #self.draw()
        photocube.cube.cubeService.coordinate.constructCube()
        self.load( photocube.cube.cubeService.coordinate.cube )
        ...
        # Create drill up label
        parent_node = photocube.cube.cubeService.get_z().get_parent_for_node( clicked_node )
    ...
    def OnClusterContextMenuSelect(self, index):
        if index == "add tag to cell":
            ...
            cell = self.cube.getCell(image.getCellKey())
            photocube.dialogs.dialogService.openAddTagToCellDialog(cell)
