...
from photocube.common import X_AXIS, Z_AXIS, Y_AXIS
...
class CubeService:
    ...
    def view_on_x(self, dim):
        if dim is None:
            ...
            x_ax = photocube.cube.axis.AxisTagset( dim, axis=X_AXIS)
            self.coordinate.set_x( x_ax )
        
        elif type(dim) == ObjectCubePython.PersistentDimension:
            x_ax = photocube.cube.axis.AxisHierarchy( dim, axis=X_AXIS )
            self.coordinate.set_x( x_ax )
    ...
    def view_on_y(self, dim):
        if dim is None:
            ...
            y_ax = photocube.cube.axis.AxisHierarchy( dim, axis = Y_AXIS )
            self.coordinate.set_y( y_ax )

        else:
            y_ax = photocube.cube.axis.AxisTagset( dim, axis = Y_AXIS )
            self.coordinate.set_y( y_ax )
    ...
    def view_on_z(self, dim):    
        if dim is None:
            ...
            z_ax = photocube.cube.axis.AxisHierarchy( dim, axis=Z_AXIS )
            self.coordinate.set_z( z_ax )
        
        else:
            z_ax = photocube.cube.axis.AxisTagset( dim, axis=Z_AXIS )
