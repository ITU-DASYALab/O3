...
from photocube.actions import actionManager, ActionFilter, ACTION_ADD, ACTION_TAGFILTER
from photocube.actions.action import ACTION_TYPE_NUMERICAL_RANGEFILTER
...
class AddTagFilterDialog(abstract.AbstractDialog):
    ...
    def contextHandler(self, index):
        ...
        if index == 'add tag filter':
            ...
            tagset = photocube.objectcube.objectCubeService.get_tagset_by_id(tagset_id)
            tag = tagset.getTag(self.selected_item_tag_id)
            
            tagFilter = TagFilter(tag, tagset.id)
            photocube.objectcube.objectCubeService.get_state().addFilter( tagFilter )
            photocube.objectcube.objectCubeService.update_state()
    ...
    def onItemclick(self, item, tagset_id, tag_id):
        ...
        photocube.menu.context_menu_handler.showTagFilterItemContectMenu()
...
class AddNewTagsetDialog(abstract.AbstractDialog):
    ...
    def onItemclick(self, item):
        self.selected_item = item
        photocube.menu.context_menu_handler.showTagsetItemContectMenu()
...
class DialogAddNumericalRange(abstract.AbstractDialog):
    ...
    def click_ok_button(self):
        try:
            ...
            photocube.objectcube.objectCubeService.get_state().addFilter( numerical_range_filter )
            photocube.objectcube.objectCubeService.update_state()
...
class DialogAddTagToCell(abstract.AbstractDialog):
    ...
    def __onTagsetDropDownSelectChange(self, t):
        selectedTagset = photocube.objectcube.objectCubeService.getTagsetByName(t)
        self.tags = []
...
class DialogAddDateFilter(abstract.AbstractDialog):
    ...
    def click_ok_button(self):        
        ...
        try:
            ...
            photocube.objectcube.objectCubeService.get_state().addFilter( filter )
            photocube.objectcube.objectCubeService.update_state()
